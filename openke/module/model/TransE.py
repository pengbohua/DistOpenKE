...
class parallel_normalize(torch.autograd.Function):
    ...
    def forward(ctx, input):
        ...
        sum_of_square = torch.sum(torch.square(input), dim=1, keepdim=True)
        dist.all_reduce(sum_of_square, dist.ReduceOp.SUM, group=group)
        inverse_l2_norm = 1 / (sum_of_square**0.5)
        ctx.save_for_backward(inverse_l2_norm, input)
        return input * inverse_l2_norm
